          ...
          <div class="card">
            ...
            <div class="card-body">
              <form action="{{ route('manageundianpacu.update', $undianPacu->id) }}" method="POST" enctype="multipart/form-data">
                ...
                {{-- Pilih Jalur Peserta --}}
                @php
                    $participantsRaw = is_array($undianPacu->participants) ? $undianPacu->participants : [];
                    $pesertaIds = $participantsRaw['peserta'] ?? [];
                    $pairing = $participantsRaw['pairing'] ?? [];
                    $defaultPeserta = old('jalur_peserta', $pesertaIds);
                @endphp
                <div class="form-group mb-3">
                    ...
                    @foreach($jalur as $j)
                      <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="jalur_peserta[]" id="jalur-check-{{ $j->id }}" value="{{ $j->id }}"
                               {{ in_array($j->id, $defaultPeserta ?: []) ? 'checked' : '' }}>
                ...
                <div class="form-group mb-3">
                  ...
                  <div class="mb-2">
                    <button type="button" class="btn btn-outline-primary w-100" id="btn-spin-jalur">
                      ...
                      Acak Ulang Seluruh Jalur Peserta (Kiri vs Kanan + Bay)
                    </button>
                            ...
                            @foreach($pairing as $idx => $p)
                                <tr>
                                  ...
                                  @if(isset($p['kiri']) && isset($p['kanan']))
                                    @php
                                        $kiriJ = $jalur->firstWhere('id', $p['kiri']);
                                        $kananJ = $jalur->firstWhere('id', $p['kanan']);
                                    @endphp
                                    <td>{{ $kiriJ->nama_jalur ?? ('ID '.$p['kiri']) }} (kiri)</td>
                                    <td>{{ $kananJ->nama_jalur ?? ('ID '.$p['kanan']) }} (kanan)</td>
                                  @elseif(isset($p['bay']))
                                    @php
                                        $bayJ = $jalur->firstWhere('id', $p['bay']);
                                    @endphp
                                    <td>-</td>
                  ...
                  <div id="participants-hidden-container">
                    @if($hasPairing)
                        @foreach($pairing as $idx => $p)
                            @if(isset($p['kiri']) && isset($p['kanan']))
                                <input type="hidden" name="participants[{{ $idx }}][kiri]" value="{{ $p['kiri'] }}">
                                <input type="hidden" name="participants[{{ $idx }}][kanan]" value="{{ $p['kanan'] }}">
                            @elseif(isset($p['bay']))
                                <input type="hidden" name="participants[{{ $idx }}][bay]" value="{{ $p['bay'] }}">
                            @endif
                        @endforeach
                    @endif
                  </div>
