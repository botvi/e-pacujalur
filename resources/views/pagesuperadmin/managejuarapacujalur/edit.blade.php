          ...
          <div class="row align-items-center">
            <div class="col-md-12">
              <ul class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">Form Edit Data Juara Pacu Jalur</li>
              </ul>
            </div>
            <div class="col-md-12">
              <div class="page-header-title">
                <h2 class="mb-0">Form Edit Data Juara Pacu Jalur</h2>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-sm-10">
          ...
          <div class="card">
            <div class="card-header">
              <h5>Form Edit Data Juara Pacu Jalur</h5>
            </div>
            <div class="card-body">
              <form action="{{ route('managejuarapacujalur.update', $juaraPacuJalur->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group mb-3">
                      ...
                      <select name="gelanggang_id" class="form-control @error('gelanggang_id') is-invalid @enderror" required>
                        ...
                        @foreach($gelanggang as $g)
                          <option value="{{ $g->id }}" {{ old('gelanggang_id', $juaraPacuJalur->gelanggang_id) == $g->id ? 'selected' : '' }}>
                            {{ $g->nama_gelanggang }}
                          </option>
                        @endforeach
                      </select>
                  ...
                  <div class="col-md-6">
                    <div class="form-group mb-3">
                      ...
                      <select name="event_id" class="form-control @error('event_id') is-invalid @enderror" required>
                        ...
                        @foreach($event as $e)
                          <option value="{{ $e->id }}" {{ old('event_id', $juaraPacuJalur->event_id) == $e->id ? 'selected' : '' }}>
                            {{ $e->nama_event }}
                          </option>
                        @endforeach
                      </select>
                ...
                <div class="form-group mb-3">
                  ...
                  <select name="tahun" class="form-control @error('tahun') is-invalid @enderror" required>
                    ...
                    @for ($th = $tahun_awal; $th <= $tahun_akhir; $th++)
                      <option value="{{ $th }}" {{ old('tahun', $juaraPacuJalur->tahun) == $th ? 'selected' : '' }}>{{ $th }}</option>
                    @endfor
                  </select>
                  @error('tahun')
                    <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    @if($daftarJuara && count($daftarJuara) > 0)
                      @foreach($daftarJuara as $index => $juara)
                        <div class="juara-item mb-3 p-3 border rounded">
                          <div class="row">
                            <div class="col-md-6">
                              <label class="form-label">Juara {{ $index + 1 }}</label>
                              <select name="daftar_juara[{{ $index }}][jalur_id]" class="form-control @error('daftar_juara.'.$index.'.jalur_id') is-invalid @enderror" required>
                                <option value="">Pilih Jalur</option>
                                @foreach($jalur as $j)
                                  <option value="{{ $j->id }}" {{ old('daftar_juara.'.$index.'.jalur_id', $juara['jalur_id']) == $j->id ? 'selected' : '' }}>
                                    {{ $j->nama_jalur }}
                                  </option>
                                @endforeach
                              </select>
                              @error('daftar_juara.'.$index.'.jalur_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                              @enderror
                            </div>
                            <div class="col-md-6 d-flex align-items-end">
                              <button type="button" class="btn btn-danger btn-sm remove-juara" {{ count($daftarJuara) == 1 ? 'style=display:none;' : '' }}>
...
@section('script')
<script>
    let juaraCount = {{ $daftarJuara ? count($daftarJuara) : 1 }};
